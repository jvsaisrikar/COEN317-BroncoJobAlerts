...
def on_message_received(ch, method, properties, body):
    print(f'Internal - received new message: {body}')
...
channel.queue_bind(exchange='routing', queue=queue.method.queue, routing_key='internal')
channel.queue_bind(exchange='routing', queue=queue.method.queue, routing_key='both')

channel.basic_consume(queue=queue.method.queue, auto_ack=True,
    on_message_callback=on_message_received)

print('Internal Starting Consuming')
